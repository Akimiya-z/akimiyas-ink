...
- 再补 1 到 2 篇真正属于你的文章
- 如果你有自己的域名，可以把 `.vercel.app` 换成自定义域名
- 每次改完一个稳定小版本就提交一次，这样以后回退会轻松很多

## 故障排除

- **问题**：手机打不开 `127.0.0.1`
  **处理**：`127.0.0.1` 只能本机访问；手机要看效果时，直接打开你的 Vercel 线上地址。

- **问题**：本地改好了，线上还是旧版本
  **处理**：先确认你已经 `git push`，再到 Vercel 看这次部署是否成功；如果改的是站点域名，还要确认 `astro.config.ts` 和 `src/site.config.ts` 里的网址已经更新。
